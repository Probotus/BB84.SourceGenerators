...
public sealed class IniFileGenerator : IIncrementalGenerator
{
  ...
  private void Execute(SourceProductionContext context, (ClassDeclarationSyntax ClassSyntax, SemanticModel SemanticModel)? input)
  {
    ...
    List<SectionInfo> sections = GetSections(classDeclaration, semanticModel);
    ...
    AppendWriteMethod(sb, className, accessibility, sections);
  ...
  private static void AppendReadMethod(StringBuilder sb, string className, string accessibility, List<SectionInfo> sections, string stringComparison)
  {
    ...
    for (int s = 0; s < sections.Count; s++)
    {
      ...
      {
        sb.AppendLine($"          if (result.{section.PropertyName} == null)");
        sb.AppendLine($"            result.{section.PropertyName} = new {section.TypeName}();");
        sb.AppendLine();
      }
      ...
      for (int v = 0; v < section.Values.Count; v++)
      {
        ...
        sb.AppendLine($"            result.{section.PropertyName}.{value.PropertyName} = {GetParseExpression(value.TypeName, "value")};");
      }
  ...
  private static void AppendWriteMethod(StringBuilder sb, string className, string accessibility, List<SectionInfo> sections)
  {
    sb.AppendLine("    /// <summary>");
    ...
    for (int s = 0; s < sections.Count; s++)
    {
      ...
      sb.AppendLine($"      if (instance.{section.PropertyName} != null)");
      sb.AppendLine("      {");
      sb.AppendLine($"        sb.AppendLine(\"[{section.SectionName}]\");");

      foreach (ValueInfo value in section.Values)
      {
        sb.AppendLine($"        sb.AppendLine(\"{value.KeyName}=\" + {GetToStringExpression($"instance.{section.PropertyName}.{value.PropertyName}", value.TypeName)});");
      }
  ...
  private static List<SectionInfo> GetSections(ClassDeclarationSyntax classDeclaration, SemanticModel semanticModel)
  {
    ...
    foreach (ISymbol member in classSymbol.GetMembers())
    {
      ...
      INamedTypeSymbol? sectionType = propertySymbol.Type as INamedTypeSymbol;
